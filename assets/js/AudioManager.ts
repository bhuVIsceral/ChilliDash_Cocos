...
export class AudioManager extends Component {
    ...
    public playChilliSfx() {
        if (this.collectChilliSfx && this.bgmSource) {
            this.bgmSource.playOneShot(this.collectChilliSfx, 0.5);
        }
    }

    public playPowerupSfx() {
        if (this.collectPowerupSfx && this.bgmSource) {
            this.bgmSource.playOneShot(this.collectPowerupSfx, 1.0);
        }
    }

    public playHitSfx() {
        if (this.hitObstacleSfx && this.bgmSource) {
            this.bgmSource.playOneShot(this.hitObstacleSfx, 1.0);
        }
    }

    public playWooshSfx() {
        if (this.failSfx && this.bgmSource) {
            this.bgmSource.playOneShot(this.failSfx, 1.0);
        }
    }
}
